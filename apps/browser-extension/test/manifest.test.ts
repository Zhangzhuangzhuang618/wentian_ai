...
test("扩展保留用户选择、本地预览和最终确认步骤", () => {
  ...
  assert.match(contentSource, /findScrollableAncestors/);
  assert.match(contentSource, /mergeReferenceLinkObservations/);
  assert.match(contentSource, /findReferenceEntryElement/);
